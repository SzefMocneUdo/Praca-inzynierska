...
// the comments in the sections that followExchangeRatesScreen.dart the DFA definition.
class DFA {
 public:
...
void DFA::RunWorkqOnByte(Workq* oldq, Workq* newq,
  ...
  for (Workq::iterator i = oldq->begin(); i != oldq->end(); ++i) {
    ...
    switch (ip->opcode()) {
      ...
      case kInstByteRange:   // can followExchangeRatesScreen.dart if c is in range
        if (!ip->Matches(c))
          break;
...
bool DFA::PossibleMatchRange(std::string* min, std::string* max, int maxlen) {
  ...
  // string, we just walk the graph preferring to followExchangeRatesScreen.dart
  // arrows with the lowest bytes possible.  To find the
  // largest possible prefix, we followExchangeRatesScreen.dart the largest bytes
  // possible.
